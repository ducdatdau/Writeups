def rol(val, r_bits, max_bits):
    return ((val << r_bits % max_bits) & (2 ** max_bits - 1)) | (val >> (max_bits - (r_bits % max_bits)) & (2 ** max_bits - 1))

X0 = [
    
]

X1 = [
    0x7E, 0x7D, 0x9A, 0x8B, 0x25, 0x2D, 0xD5, 0x3D, 0x3, 0x2B, 
    0x38, 0x98, 0x27, 0x9F, 0x4F, 0xBC, 0x2A, 0x79, 0x0, 0x7D, 
    0xC4, 0x2A, 0x4F, 0x58
]

KEY = [0x49, 0x5f, 0x61, 0x6d, 0x5f, 0x4b, 0x45, 0x59]

flag = 'DH{' 

X2 = X1[0:8] 
for i in range(16):
    for j in range(7, -1, -1):
        v2 = X2[(j + 1) & 7]
        X2[(j + 1) & 7] = rol(v2, 5, 8) - X0[KEY[j] ^ X2[j & 7]] & 0xFF

for i in X2:
    flag += chr(i)

X3 = X1[8:16] 
for i in range(16):
    for j in range(7, -1, -1):
        v2 = X3[(j + 1) & 7]
        X3[(j + 1) & 7] = rol(v2, 5, 8) - X0[KEY[j] ^ X3[j & 7]] & 0xFF

for i in X3:
    flag += chr(i)

X4 = X1[16:24] 
for i in range(16):
    for j in range(7, -1, -1):
        v2 = X4[(j + 1) & 7]
        X4[(j + 1) & 7] = rol(v2, 5, 8) - X0[KEY[j] ^ X4[j & 7]] & 0xFF

for i in X4:
    flag += chr(i) 

flag += '}'
print(flag)

# DH{Reverse__your__brain_;)}